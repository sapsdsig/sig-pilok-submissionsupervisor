...
const supervisorsToForm = (items: StoredSupervisor[]) =>
  items.map((item) =>
    item.source === "baseline"
      ? {
          ...
          ktp: { kind: "not-required" as const },
        }
      : {
          supervisorId: item.supervisorId,
...
function App() {
  ...
  const loadMasters = useCallback(async () => {
    ...
    try {
      setDistributors(await distributorService.getDistributors());
    } catch (error) {
      setDistributors([]);
      setMasterError(
  ...
  const loadIdentity = useCallback(
    async (namaDistributor: string, ap: string) => {
      ...
      try {
        const result = await submissionService.findByDistributorAp(
          namaDistributor,
          ap,
        );
        if (
          getValues("namaDistributor") !== namaDistributor ||
          getValues("ap") !== ap
        )
          return;
        if (result.exists) {
          ...
          reset({
            namaDistributor,
            ap,
            supervisors: supervisorsToForm(result.supervisors),
          });
          setMode({ kind: "create" });
  ...
  const applyDistributor = useCallback(
    async (namaDistributor: string) => {
      ...
      try {
        ...
        setMasterError(
          error instanceof ApiClientError
            ? error.message
            : "Daftar AP gagal dimuat. Silakan coba kembali.",
        );
        setLookupStatus("error");
      } finally {
        setLoadingAp(false);
  ...
  const selectDistributor = (value: string) => {
    if (value !== getValues("namaDistributor"))
      requestSwitch({ kind: "distributor", value });
  };
  ...
  const onSubmit = async (values: SupervisorFormValues) => {
    ...
    try {
      ...
      const payload: SubmissionRequest = {
        ...
        namaDistributor: normalized.namaDistributor,
        ap: normalized.ap,
        submissionAreaId: normalized.submissionAreaId,
        supervisors,
      };
      ...
      if (mode.kind === "edit") {
        const refreshed = await submissionService.findByDistributorAp(
  ...
  return (
    <FormShell
      ...
      subtitle="Pendataan Supervisor berdasarkan Distributor dan AP."
    >
      {successResult ? (
            ...
            {ready && (
              <SectionCard>
                <SectionHeader
                  step={2}
                  title="Supervisor"
                  description="Data yang ditampilkan pada menu ini merupakan hasil pengisian data pada periode HY 2026."
                />
                <div className="mt-6">
                  <WilayahCard />
                </div>
                <FieldError
                  message={
                    typeof errors.supervisors?.message === "string"
                ...
                feedback={
                  submissionError ? (
                    ...
                    <StatusBanner variant="info" compact>
                      Mengunggah KTP Supervisor {processingStatus.supervisorNo}:{" "}
      ...
      <ConfirmationDialog
        ...
        title={
          pendingSwitch?.kind === "ap" ? "Ganti AP?" : "Ganti Distributor?"
        }
        description="Perubahan yang belum disimpan akan dibuang sebelum data lain dimuat."
        confirmLabel="Ganti"
        ...
        onConfirm={() => {
          ...
          if (!next) return;
          if (next.kind === "distributor") void applyDistributor(next.value);
          else void loadIdentity(getValues("namaDistributor"), next.value);
        }}
      />
    </FormShell>
