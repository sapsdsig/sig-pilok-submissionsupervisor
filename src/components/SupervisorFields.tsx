...
export function SupervisorFields(props: Props) {
  ...
  const namePath =
    `supervisors.${props.supervisorIndex}.namaSupervisor` as const;
    ...
    <div className="supervisor-panel w-full min-w-0 max-w-full">
      <div className="mb-4 flex min-w-0 items-center justify-between gap-3">
        ...
        <button
          ...
          title={
            props.canRemove ? "Hapus Supervisor" : "Minimal satu Supervisor"
          }
      ...
      <div
        className={`grid w-full min-w-0 grid-cols-1 gap-5 ${baseline ? "" : "lg:grid-cols-2"}`}
      >
        ...
        {!baseline && (
          <FileUploadField
            inputId={`ktp-supervisor-${props.supervisorIndex}`}
            name={ktpPath}
          />
        )}
      </div>
