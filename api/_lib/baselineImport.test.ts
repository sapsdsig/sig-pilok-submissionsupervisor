...
import {
  buildBaselineImportPlan,
  extractSavedCombinations,
  mapMasterToBaseline,
  runBaselineImport,
} from "./baselineImport.js";
...
describe("HY baseline import", () => {
  ...
  it("is idempotent and skips saved Distributor + AP combinations", () => {
    const plan = buildBaselineImportPlan(
      master,
      table([
        {
          rowNumber: 2,
    ...
    expect(plan.skippedSavedCombination).toBe(2);
  ...
  it("dry-run writes nothing while explicit apply writes intended rows only", async () => {
    ...
    expect(write).toHaveBeenCalledWith(
      expect.arrayContaining([expect.objectContaining({ id_mdxl: "1001" })]),
    );
  });
  it("derives saved combinations from parent + AP and rejects ambiguous legacy parents", () => {
    const parents = table([
      {
        rowNumber: 2,
        record: { submission_id: "SUP-1", nama_distributor: "VENDOR A" },
      },
    ]);
    expect(
      extractSavedCombinations(
        parents,
        table([
          { rowNumber: 2, record: { submission_id: "SUP-1", ap: "AP1" } },
        ]),
      ),
    ).toEqual([{ namaDistributor: "VENDOR A", ap: "AP1" }]);
    expect(() => extractSavedCombinations(parents, table())).toThrow(
      "Migrasi manual",
    );
  });
});
